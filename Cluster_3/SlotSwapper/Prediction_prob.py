...
def main():
    ...
    for i in range(100):
        ...
        Select_10_schedules.append(Base_Schedule_list[num - 1])
        Select_10_FSeq.append(Base_Flow_list[num - 1])
    ...
    for i in range(100):
        for j in range(60):
            for k in range(6):
                ...
                Links.append(Select_10_schedules[i][j])
                PP[i][j].append(0)       
        
    for i in range(100):
        for j in range(60):
            if i > 0:
                if Select_10_FSeq[i][j] != "-":
                    s = int(Select_10_FSeq[i][j])
                    #print s
                    Predict[i][j][s - 1] += 1
                     
    f7 = open("../Util_" + str(d) + "/PP_Slot_" + str(d) + ".txt","w")           
    for i in range(100):
        for j in range(60):
            for k in range(6):
                sum = 0
                if i > 0:
                    for l in range(i-1):
                        if Select_10_schedules[i][j][0] != -1 and Select_10_schedules[i][j][1] != -1 and Select_10_schedules[i][j][0] == Select_10_schedules[l][j][0] and Select_10_schedules[i][j][1] == Select_10_schedules[l][j][1]:
                            #print(Select_10_schedules[i][j])
                            #print(Select_10_schedules[l][j])
